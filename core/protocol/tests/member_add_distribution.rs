...
fn newly_added_member_can_decrypt_messages_sent_after_joining() {
    ...
    let mut group = GroupSession::new(sender.public())
        .add_member(GroupMember(original.public()));

    // A new member is added.
    let new_member = IdentityKeyPair::generate();
    group = group.add_member(GroupMember(new_member.public()));

    let ciphertext_after = group.encrypt_as(&sender, b"hello, new member!").expect("encrypt");

    let plain = group.decrypt_as(&new_member, &ciphertext_after).expect("new member decrypts post-join");
...
fn newly_added_member_cannot_decrypt_messages_sent_before_joining() {
    ...
    let ciphertext_before = group.encrypt_as(&sender, b"old message, before you joined").expect("encrypt");
