...
use protocol::fanout::{DeviceId, FanoutSession};
#[test]
fn message_to_multi_device_recipient_decrypts_on_every_linked_device() {
    ...
    let recipient_c = generate_identity_key_pair();
    ...
    let mut fanout = FanoutSession::establish(&sender, &devices).expect("fanout session");
    let ciphertexts = fanout
        .encrypt_to_all(b"hi to all my devices")
        .expect("encrypt fanout");

    assert_eq!(ciphertexts.len(), 3, "one ciphertext per linked device");
    assert_eq!(
        fanout.decrypt_as(&recipient_a, &ciphertexts[0]).unwrap(),
        b"hi to all my devices"
    );
    assert_eq!(
        fanout.decrypt_as(&recipient_b, &ciphertexts[1]).unwrap(),
        b"hi to all my devices"
    );
    assert_eq!(
        fanout.decrypt_as(&recipient_c, &ciphertexts[2]).unwrap(),
        b"hi to all my devices"
    );
}
